...
class geom_confs(Dataset):
    ...
    def featurize_mol(self, mol_dic):
        ...
        z = [atom.GetAtomicNum() for atom in correct_mol.GetAtoms()]
        graph = mol2graph(correct_mol)
        ...
        data = Data(x=x, z=z, pos=pos, edge_index=edge_index, edge_attr=edge_attr, num_nodes=num_nodes, pos_mask=pos_mask)
...
def construct_loader(args, modes=('train', 'val')):
    ...
    for mode in modes:
        ...
        loader = DataLoader(dataset, batch_size=16, shuffle=False if mode == 'test' else True)
        """
