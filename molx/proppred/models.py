...
class E_GCL_mask(E_GCL):
    ...
    def forward(self, h, edge_index, coord, node_mask, edge_mask, edge_attr=None, node_attr=None, n_nodes=None):
        ...
        #print("edge_feat: ", edge_feat, edge_feat.shape)
...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=4, coords_weight=1.0, attention=False, node_attr=1):
        ...
        self.graph_dec = nn.Sequential(nn.Linear(self.hidden_nf, self.hidden_nf // 2),
                                       act_fn,
    ...
    def forward(self, h0, x, edges, edge_attr, node_mask, edge_mask, n_nodes, batch):
        ...
        h = self.node_dec(h)
        #print("h: ", h, h.shape)
        #print("node_mask: ", node_mask, node_mask.shape)
        h = h * node_mask
        h = h.view(-1, n_nodes, self.hidden_nf)
        #h = h.view(-1, self.hidden_nf)
        #print("h before sum: ", h, h.shape)
        h = torch.sum(h, dim=1)
        #h = global_add_pool(h, batch)
        #print("h after: ", h, h.shape)
